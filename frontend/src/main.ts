...
function render() {
  if (!state) {
    ...
    document.getElementById('join-btn')!.onclick = () => {
      const input = (document.getElementById('room-input') as HTMLInputElement).value;
      if (input) {
  ...
  if (state.status === "waiting") {
    app.innerHTML = `
      ...
      <ul style="list-style:none;padding:0;">
        ${state.players.map((p: string) => `<li>${p}</li>`).join("")}
      </ul>
  ...
  let html = `
    ...
    <div class="logs">
      ${state.logs.map((l: string) => `<div>${l}</div>`).join("")}
    </div>
  ...
  html += `
    <div class="player-area">
      ...
      <h4>My Hand</h4>
      <div class="hand">
        ${state.my_hand.map((card: [string, number], i: number) => {
